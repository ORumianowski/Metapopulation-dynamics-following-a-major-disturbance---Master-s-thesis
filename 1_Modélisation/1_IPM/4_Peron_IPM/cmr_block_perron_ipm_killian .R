...
#marr <- marray(cormorant$ms.ch, unobs=5)
...
myconsts <- list(n.years=ncol(cormorant$ms.ch), n.colony = n.colony, ns=ns,
                 cst_pr = ((n.years-1)*ns))
...
mycode = nimbleCode(code ={
  ...
  for (t in 1:(n.years-2)){
    ...
    for (j in (t+1):(n.years-1)){
      for (s1 in 1:ns){
        for (s2 in 1:ns){
           U[(t-1)*ns+(s1), (j-1)*ns+(s2)] <- U[(t-1)*ns+(s1), (j-2)*ns+(s2)] %*% psi[s1,s2] %*% dq[s1,t,s2]
        }
      }
    } #j
  } #t
  ...
  for (t in 1:(n.years-2)){
    for (s1 in 1:ns){
      for (s2 in 1:ns){  
         pr[(t-1)*ns+(s1),(t-1)*ns+(s2)] <- U[(t-1)*ns+(s1),(t-1)*ns+(s2)] %*% psi[s1,s2] %*% dp[s1,t,s2]
      }
    }
    # Above main diagonal
    for (j in (t+1):(n.years-1)){
      for (s1 in 1:ns){
        for (s2 in 1:ns){
      pr[(t-1)*ns+(s1), (j-1)*ns+(s2)] <- U[(t-1)*ns+(s1), (j-1)*ns+(s2)] %*% psi[s1,s2] %*% dp[s1,j,s2]
        }
      } 
    } #j
  } #t
  ...
  for (s1 in 1:ns){
    for (s2 in 1:ns){
      pr[(n.years-2)*ns+(s1), (n.years-2)*ns+(s2)] <- psi[s1,s2] %*% dp[s1,n.years-1,s2] 
    }
  }
...
inits <- function(cc=cormorant$count){
  ...
  B[4,1] <- rpois(1, cc[4,1])
  B[5,1] <- rpois(1, cc[5,1])
  N <- B * 0.55/0.45
  return(list(B=B, N=N))
}
