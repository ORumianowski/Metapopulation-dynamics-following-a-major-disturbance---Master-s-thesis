...
myconsts <- list(rel.j=rowSums(marr[,,1]), rel.a=rowSums(marr[,,2]),
                 n.occasions=dim(marr)[2], len_J = length(woodchat5$repro[,1]))
...
mycode = nimbleCode(code ={
  ...
  for (t in 1:(n.occasions-1)){
    marr.j[t,1:n.occasions] ~ dmulti(pr.j[t,1:n.occasions], rel.j[t])  # pbm de conversion en nimble
...
# MCMC settings
ni <- 20000; nb <- 4000; nc <- 3; nt <- 2; na <- 3000
